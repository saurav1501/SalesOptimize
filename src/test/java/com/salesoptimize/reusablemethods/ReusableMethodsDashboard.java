...
public class ReusableMethodsDashboard extends BaseClass{
	...
	public void selectBusinessCountry(int rowNum, String country) throws Exception {
		...
		CommonMethod.elementToBeClickable("businessdropdown", minWait);
		CommonMethod.click("businessdropdown");
		
		CommonMethod.scrolldowntoElement("businessCountryPuls");
		Thread.sleep(2000);
		CommonMethod.elementToBeClickable("businessCountryPuls", minWait);
		...
		Assert.assertEquals(CommonMethod.getText("BusinessCountryText"), data.getCellData(country,"BusinessLocation",rowNum),"Success");
	...
	public void selectIndustryRetailAndFashiClothing(int rowNum, String country) throws Exception {
		CommonMethod.testlog("Pass","Test Starated");
		CommonMethod.elementToBeClickable("industryDropdown", minWait);
		CommonMethod.click("industryDropdown");
		
		CommonMethod.elementToBeClickable("IndustoryPlus", minWait);
...
public void searchButtonAndCaptureResult(int rowNum, String searchresult) throws Exception {
	...
	Assert.assertEquals(CommonMethod.getText("numberOfResult").contains(data.getCellData(searchresult, "numberOfResult", rowNum)),true);
	CommonMethod.testlog("Pass","Test Passed");
}
...
public void firstRecordCaptureDetail(int rowNum, String country) throws Exception {
	...
	for(int i=2;i<=11;i++) {
		if(i==9) {
			...
			int j= i;
			j = j-1;
			data.setCellData(country, "CompanyDetails", j, driver.findElement(By.xpath("//[@id='so-vue-app']/div/div[1]/div[1]/div/div/div/table/tbody/tr["+i+"]/td[1]")).getText());
			data.setCellData(country, "Values", j, driver.findElement(By.xpath("//[@id='so-vue-app']/div/div[1]/div[1]/div/div/div/table/tbody/tr["+i+"]/td[2]")).getText());
			
		}
		else {
		System.out.println(data.setCellData(country, "CompanyDetails",i, driver.findElement(By.xpath("//[@id='so-vue-app']/div/div[1]/div[1]/div/div/div/table/tbody/tr["+i+"]/td[1]")).getText()));
		System.out.println(data.setCellData(country, "Values", i, driver.findElement(By.xpath("//[@id='so-vue-app']/div/div[1]/div[1]/div/div/div/table/tbody/tr["+i+"]/td[2]")).getText()));
